...
bool isCentersNear(Rect r1, Rect r2){
	...
	return ( norm(firstCenter - secondCenter) < (0.5 * min(r1.width, r2.width)) );
}
    ...
    for(int i = 0; i < (int)groundtruth.size(); i++){
        ...
        for(int rectIdx = 0; rectIdx < (int)outputs.size(); rectIdx++){
            ...
            for (int annoIdx = 0; annoIdx < (int)current_regions.size(); annoIdx++){
                        ...
                        if( ((surface_overlap > 0.5 * surface_det) && (surface_overlap > 0.5 * surface_anno)) || (isCentersNear(region_det, region_anno) && (surface_overlap > (0.9 * surface_det))) ){
                             TP++;
