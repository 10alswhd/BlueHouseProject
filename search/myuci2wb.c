...
// Set VARIANTS for in WinBoard variant feature.
// (With -s option this will always be reset to use "shogi".)
...
int StartEngine(const char *, const char *);
void GUI2Engine(void);
void Engine2GUI(void);
void StartSearch(char *);
void StopPonder(int);
void LoadPos(int);

char move[2000][10];
char checkOptions[8192];
char iniPos[256];
char hashOpt[20];
char pause, pondering, ponder, post, hasHash, c;
char sc='c';
int mps, tc, inc, stime, depth, myTime, hisTime, stm;
int computer = NONE;
int memory;
int oldMem = 0;
int cores, moveNr, lastDepth, lastScore, startTime;
int statDepth, statScore, statNodes, statTime, currNr;
/* for analyze mode */
char currMove[20];
...
main(int argc, char **argv)
{
	...
	if (argc == 2 && !strcmp(argv[1], "-v")) {
		printf("UCI2WB " VERSION " by H.G.Muller\n");
		return EXIT_SUCCESS;
	}
	if (argc > 1 && argv[1][0] == '-') {
		sc = argv[1][1];
	...
	if (argc < 2) {
		printf("usage is: U%cI2WB [-s] <engine.exe>"
		       " [<engine directory>]", sc - 32);
		return EXIT_FAILURE;
	}
	if (argc > 2)
		dir = argv[2];
	...
	if ((errno = StartEngine(argv[1], dir)) != NO_ERROR) {
		perror(argv[1]);
		return EXIT_FAILURE;
	}
...
int
StartEngine(const char *cmdLine, const char *dir)
{
	...
	fromE = (FILE *)_fdopen(_open_osfhandle((long)hChildStdoutRd,
						_O_TEXT|_O_RDONLY), "r");
	toE   = (FILE *)_fdopen(_open_osfhandle((long)hChildStdinWr,
						_O_WRONLY), "w");
	...
	for (;;) {
		...
		if (!strcmp(command, "bestmove")) {
			if (pause) {
				pondering = pause = 0;
				continue;
			} // bestmove was reply to ponder miss or analysis result; ignore.
			// move was a move to be played
			if (strstr(line+9, "resign")) {
				printf("resign\n");
				computer = NONE;
			}
			if (strstr(line+9, "(none)") ||
			    strstr(line+9, "null") ||
			    strstr(line+9, "0000")) {
				printf("%s\n",
				       lastScore < -99999 ? "resign" : "1/2-1/2 {stalemate}");
				computer = NONE;
			}
			...
			if (mps && ((moveNr+1)/2) % mps == 0)
				myTime += tc;
			if (stime)
				myTime = stime; // new session or move starts
			...
			if (!post)
				continue;
			if (strstr(line, "info string ") == line)
				printf("%d 0 0 0 %s", lastDepth, line+12);
			else {
				...
				if (p = strstr(line+4, " score cp ")) {
					sscanf(p+10, "%d", &s);
					statScore = s;
				} else if (p = strstr(line+4, " score mate ")) {
					sscanf(p+12, "%d", &s);
					s += s>0 ? 100000 : -100000;
					statScore = s;
				} else if (p = strstr(line+4, " score ")) {
					sscanf(p+7, "%d", &s);
					statScore = s;
				}
				...
				if (p = strstr(line+4, " currmove ")) {
					sscanf(p+10, "%s", currMove);
				}
				if (p = strstr(line+4, " currmovenumber ")) {
					sscanf(p+16, "%d", &currNr);
				}
				if (pv = strstr(line+4, " pv ")) {
					// convert PV info to WB thinking output
					printf("%3d  %6d %6d %10d %s",
					       lastDepth=d, lastScore=s, t, n, pv+4);
				}
			}
		}
...
nomove:
		...
		if (!strcmp(command, "new")) {
			...
			if (memory != oldMem && hasHash)
				 fprintf(toE, "setoption name %s value %d\n",
					 hashOpt, memory);
			oldMem = memory;
		...
		else if (!strcmp(command, "cores")) {
			sscanf(line, "cores %d", &cores);
		}
...
StartSearch(char *ponder)
{
	...
	nr = moveNr + (ponder[0] != 0);

	fprintf(toE, "\ngo wtime %d btime %d",
		stm == WHITE ^ sc=='s' ? myTime : hisTime,
		stm == BLACK ^ sc=='s' ? myTime : hisTime);
	printf("\n# go wtime %d btime %d",
	       stm == WHITE ^ sc=='s' ? myTime : hisTime,
	       stm == BLACK ^ sc=='s' ? myTime : hisTime);

	if (stime > 0) {
		fprintf(toE, " movetime %d", stime);
		printf(" movetime %d", stime);
	} else if (mps)
		fprintf(toE, " movestogo %d", mps*(nr/(2*mps)+1)-nr/2);
		printf(" movestogo %d", mps*(nr/(2*mps)+1)-nr/2);
	}
	if (inc) {
		fprintf(toE, " winc %d binc %d", inc, inc);
		printf(" winc %d binc %d", inc, inc);
	}
	if(depth > 0) {
		fprintf(toE, " depth %d", depth);
...
StopPonder(int pondering)
{
	...
	/*
	 * note: 'pondering' remains set until engine acknowledges 'stop'
	 * with 'bestmove'
	 */
	fprintf(toE, "stop\n");
	printf("# stop\n");
